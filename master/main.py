...
class CrawlerCLI:
    ...
    def _get_kafka_admin_client(self) -> KafkaAdminClient:
        ...
        try:
            ...
            raise ConnectionError(
                f"Unable to connect to Kafka brokers at {self.bootstrap_servers}")
    ...
    def _check_worker_containers(self) -> List[Dict[str, Any]]:
        ...
        for i in range(1, 6):  # worker-1 to worker-5
            ...
            try:
                # Try to create a consumer for the topic to test if workers are
                # connected
                consumer = self._get_kafka_consumer(topic)
    ...
    def start_crawling(self) -> None:
        ...
        # We're running inside the master container, so assume infrastructure
        # is ready
    ...
    try:
        ...
        if args.command == 'start':
            ...
            if not args.urls:
                ...
                print(
                    "Usage: python3 main.py add_url --urls <url1> [url2] ... [--priority N] [--queue QUEUE]")
                sys.exit(1)

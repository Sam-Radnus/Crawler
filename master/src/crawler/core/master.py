    ...
    def _create_producer(self) -> KafkaProducer:
        ...
        for delay in backoff_seconds:
            try:
                ...
                last_error = e
                print(
                    f"⚠️  Kafka broker not available. Retrying in {delay}s...")
                time.sleep(delay)
    ...
    def dispatch(self) -> None:
        ...
        for url in seed_urls:
            try:
                ...
                if self.robots_checker and not self.robots_checker.can_fetch(
                        url):
                    print(
                        f"⚠️  Skipping URL (disallowed by robots.txt): {url}")
                    skipped += 1
                    continue

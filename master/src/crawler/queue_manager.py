...
class URLQueueManager:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.robots_checker = RobotsChecker(
            user_agent="WebCrawler/1.0") if respect_robots else None
    ...
    def add_url(
        ...
        try:
            ...
            # Mark as 'manual' source so workers process immediately without
            # aging delay
            payload = {
                "url": url,
                "priority": priority,
    ...
    def add_urls_batch(self,
                       urls: List[str],
                       priority: Optional[int] = None,
                       queue: Optional[str] = None) -> Dict[str,
                                                            bool]:
        """
...
def main():
    ...
    batch_parser = subparsers.add_parser(
        'batch', help='Add multiple URLs from file')
    batch_parser.add_argument(
        'file', help='File containing URLs (one per line)')
    batch_parser.add_argument(
        '--priority',
        '-p',
        type=int,
    ...
    try:
        ...
        if args.command == 'add':
            results = manager.add_urls_batch(
                args.urls, args.priority, args.queue)

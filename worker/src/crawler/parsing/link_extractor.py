    ...
    def extract_links(self, html_content: str, base_url: str) -> Set[str]:
        ...
        try:
            ...
            if len(links) == 0:
                ...
                if all_links:
                    sample_links = [link.get('href', '')
                                    for link in all_links[:5]]
                    self.logger.warning(f"Sample hrefs: {sample_links}")
            else:
                self.logger.info(
                    f"Extracted {len(links)} links from {base_url}")
                # Log a few sample property links
                property_links = [
                    link for link in links if any(
                        pattern in link.lower() for pattern in [
                            '/property/',
                            '/pad?',
                            '/apartments/',
                            'oodle.com',
                ...
                if property_links:
                    self.logger.info(
                        f"Found {len(property_links)} potential property links: {property_links[:3]}")

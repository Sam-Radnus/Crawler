...
class UrlPrefixFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if current_url:
            # Ensure we prefix the formatted message and reset args to avoid
            # double formatting
...
class CrawlerLogger:
    ...
    def _setup_logging(self, log_level: str):
        ...
        # Configure root logger so all module loggers use the same
        # handlers/filters
    ...
    def _print_stats(self):
        ...
        with self.lock:
            ...
            # Calculate rates
            pages_per_second = self.stats['pages_crawled'] / \
                elapsed_time if elapsed_time > 0 else 0
            recent_pages_per_second = (self.stats['pages_crawled'] - self.stats.get(
                'last_pages_crawled', 0)) / time_since_last if time_since_last > 0 else 0
            ...
            self.logger.info(
                f"Recent rate: {recent_pages_per_second:.2f} pages/sec")

            if self.stats['pages_crawled'] > 0:
                success_rate = (
                    self.stats['pages_successful'] / self.stats['pages_crawled']) * 100
